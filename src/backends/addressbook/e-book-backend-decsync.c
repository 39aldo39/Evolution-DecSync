...
/* e-book-backend-decsync.c - DecSync contact backend.
...
#define E_BOOK_BACKEND_DECSYNC_VERSION "0.2"

#define E_BOOK_BACKEND_DECSYNC_REVISION_NAME "PAS-DB-REVISION"
...
static gboolean	book_backend_decsync_refresh_start (EBookBackendDecsync *bf);
...
static void
maybe_delete_uri (EBookBackendDecsync *bf,
                  const gchar *uri)
{
	GError *error = NULL;
	gchar  *filename;
...
static void
maybe_delete_unused_uris (EBookBackendDecsync *bf,
...
static gchar *
e_book_backend_decsync_extract_path_from_source (ESourceRegistry *registry,
                                              ESource *source,
...
static gchar *
safe_name_for_photo (EBookBackendDecsync *bf,
                     EContact *contact,
                     EContactPhoto *photo,
                     EContactField field)
{
...
static gchar *
hard_link_photo (EBookBackendDecsync *bf,
                 EContact *contact,
                 EContactField field,
...
static gboolean
is_backend_owned_uri (EBookBackendDecsync *bf,
                      const gchar *uri)
{
	gchar     *filename;
	gchar     *dirname;
...
maybe_transform_vcard_field_for_photo (EBookBackendDecsync *bf,
                                       EContact *old_contact,
                                       EContact *contact,
                                       EContactField field,
                                       GError **error)
 ...
 */
static PhotoModifiedStatus
maybe_transform_vcard_for_photo (EBookBackendDecsync *bf,
                                 EContact *old_contact,
                                 EContact *contact,
                                 GError **error)
{
...
static gchar *
e_book_backend_decsync_new_revision (EBookBackendDecsync *bf,
 ...
 */
static gboolean
e_book_backend_decsync_bump_revision (EBookBackendDecsync *bf,
                                   GError **error)
{
	...
	new_revision = e_book_backend_decsync_new_revision (bf, TRUE);
...
static void
e_book_backend_decsync_load_revision (EBookBackendDecsync *bf)
{
	GError *error = NULL;
	...
	if (!e_book_sqlite_get_key_value (bf->priv->sqlitedb,
		...
		e_book_backend_decsync_bump_revision (bf, NULL);
	}
}

static void
e_book_backend_decsync_load_locale (EBookBackendDecsync *bf)
{
	GError *error = NULL;
...
static void
set_revision (EBookBackendDecsync *bf,
              EContact *contact)
{
	gchar *rev;

	rev = e_book_backend_decsync_new_revision (bf, FALSE);
 ...
 ****************************************************************/
static void
cursors_contact_added (EBookBackendDecsync *bf,
                       EContact *contact)
{
	GList *l;
...
static void
cursors_contact_removed (EBookBackendDecsync *bf,
                         EContact *contact)
{
	GList *l;
           ...
           gboolean update_decsync)
{
	...
	for (ii = 0; ii < length; ii++) {
		...
		if (uids != NULL && uids[ii] != NULL)
			contact = e_contact_new_from_vcard_with_uid (vcards[ii], uids[ii]);
		else
		...
		if (e_contact_get_const (contact, E_CONTACT_UID) == NULL) {
			id = e_book_backend_decsync_create_unique_id ();
...
typedef struct {
	...
	GThread *thread;
	EFlag *running;
} DecsyncBackendSearchClosure;

static void
closure_destroy (DecsyncBackendSearchClosure *closure)
{
...
static DecsyncBackendSearchClosure *
init_closure (EDataBookView *book_view,
              EBookBackendDecsync *bf)
{
	DecsyncBackendSearchClosure *closure = g_new (DecsyncBackendSearchClosure, 1);
	...
	g_object_set_data_full (
		G_OBJECT (book_view),
		"EBookBackendDecsync.BookView::closure",
...
book_view_thread (gpointer user_data)
{
	...
	DecsyncBackendSearchClosure *closure;
	EBookBackendDecsync *bf;
...
static void
book_backend_decsync_dispose (GObject *object)
{
	EBookBackendDecsync *bf;

	bf = E_BOOK_BACKEND_DECSYNC (object);
	...
	G_OBJECT_CLASS (e_book_backend_decsync_parent_class)->dispose (object);
}

static void
book_backend_decsync_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (e_book_backend_decsync_parent_class)->finalize (object);
}

static gchar *
book_backend_decsync_get_backend_property (EBookBackend *backend,
                             ...
                             GError **error)
{
	...
	g_idle_add ((GSourceFunc) book_backend_decsync_refresh_start, bf);
                                                     ...
                                                     gboolean update_decsync)
{
	...
	for (ii = 0; ii < length && status != STATUS_ERROR; ii++) {
		...
		if (uids != NULL && uids[ii] != NULL)
			mod_contact = e_contact_new_from_vcard_with_uid (vcards[ii], uids[ii]);
		else
	...
	if (status != STATUS_ERROR) {
		if (!e_book_backend_decsync_bump_revision (bf, error))
			status = STATUS_ERROR;
	}
                              ...
                              EDataBookView *book_view)
{
	...
	closure = init_closure (book_view, E_BOOK_BACKEND_DECSYNC (backend));
...
static void
book_backend_decsync_stop_view (EBookBackend *backend,
                             EDataBookView *book_view)
{
	DecsyncBackendSearchClosure *closure = get_closure (book_view);
	gboolean need_join;
...
book_backend_decsync_get_direct_book (EBookBackend *backend)
{
	...
	dirname = e_book_backend_decsync_extract_path_from_source (
		registry, source, GET_PATH_DB_DIR);

	/* Support in-tree testing / relocated modules */
	if (modules_env) {
		backend_path = g_build_filename (
			modules_env, "libebookbackenddecsync.so", NULL);
	} else {
		backend_path = g_build_filename (
			BACKENDDIR, "libebookbackenddecsync.so", NULL);
	}

	direct = e_data_book_direct_new (
		backend_path, "EBookBackendDecsyncFactory", dirname);
...
static void
book_backend_decsync_configure_direct (EBookBackend *backend,
                                    const gchar *config)
{
	EBookBackendDecsyncPrivate *priv;
...
static void
book_backend_decsync_vcard_changed (EbSqlChangeType change_type,
                                 const gchar *uid,
                                 const gchar *extra,
                                 const gchar *vcard,
                                 gpointer user_data)
...
static gboolean
book_backend_decsync_set_locale (EBookBackend *backend,
                              const gchar *locale,
                              GCancellable *cancellable,
                              GError **error)
{
...
static gchar *
book_backend_decsync_dup_locale (EBookBackend *backend)
{
	EBookBackendDecsync *bf = E_BOOK_BACKEND_DECSYNC (backend);
	gchar *locale;
...
book_backend_decsync_create_cursor (EBookBackend *backend,
...
static gboolean
book_backend_decsync_delete_cursor (EBookBackend *backend,
                                 EDataBookCursor *cursor,
                                 GError **error)
{
	EBookBackendDecsync *bf = E_BOOK_BACKEND_DECSYNC (backend);
...
updateContacts (const gchar *uid, const gchar *vcard, Extra *extra)
{
	...
	g_rw_lock_reader_lock (&(bf->priv->lock));
	success = e_book_sqlite_has_contact (bf->priv->sqlitedb, uid, &exists, NULL);
	g_rw_lock_reader_unlock (&(bf->priv->lock));
	if (!success) return;
...
static gboolean
book_backend_decsync_refresh_start (EBookBackendDecsync *bf)
{
	ESource *source;
	...
	source = e_backend_get_source (E_BACKEND (bf));
	...
	if (interval_in_minutes > 0) {
		e_named_timeout_add_seconds (interval_in_minutes * 60, book_backend_decsync_refresh_cb, bf);
	}
	return FALSE;
}
...
static gboolean
book_backend_decsync_initable_init (GInitable *initable,
                                 GCancellable *cancellable,
                                 GError **error)
{
	EBookBackendDecsyncPrivate *priv;
	...
	if (priv->sqlitedb == NULL) {
		...
		priv->sqlitedb = e_book_sqlite_new_full (
			...
			NULL,
			book_backend_decsync_vcard_changed,
	...
	/* Load the locale */
	e_book_backend_decsync_load_locale (E_BOOK_BACKEND_DECSYNC (initable));

	/* Resolve the photo directory here. */
	priv->photo_dirname =
		e_book_backend_decsync_extract_path_from_source (
...
static void
e_book_backend_decsync_class_init (EBookBackendDecsyncClass *class)
{
...
static void
e_book_backend_decsync_initable_init (GInitableIface *iface)
{
	iface->init = book_backend_decsync_initable_init;
}
